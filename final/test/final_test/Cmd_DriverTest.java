...
{
	String[] input = {"C:\\Users\\Yoonho\\eclipse-workspace\\final\\src\\final_test\\sample.txt"};
	String[] input_QX = {"C:\\Users\\Yoonho\\eclipse-workspace\\final\\src\\final_test\\sample1.txt"};
	String[] input2 = {"C:\\Users\\Yoonho\\eclipse-workspace\\final\\src\\final_test\\empty.txt"};
	
	File_Buffer F_B = new File_Buffer();
	File_Buffer F_B_QX = new File_Buffer();
	File_Buffer F_B2 = new File_Buffer();
	
    Init_Exit I_E, I_E_QX, I_E2;
	...
	protected void setUp() throws Exception 
    {
	    ...
	    I_E_QX = new Init_Exit(input_QX, F_B_QX);
	    I_E2 = new Init_Exit(input2, F_B2);
	    RunC = new Cmd_Driver();
	...
	{
		int nl = F_B_QX.NumLins();

        F_B.setUpdateFlag(true);
        	
        try {
            RunC.Cmd_D(F_B_QX, 1);
			I_E_QX.Do_Update(F_B_QX);	
			assertEquals(nl-1, F_B_QX.NumLins());
			assertNotEquals("@Keyword", F_B_QX.GetLine(1)); 
		} catch (IOException e) {
			e.printStackTrace();
		}	
	}
	...
	public void testCmd_Q2() 
	{
        ...
        try {
        	F_B_QX.AddLine(1, "@Keyword");
        	F_B_QX.AddLine(2, "@Keyword");
        	I_E_QX.Do_Update(F_B_QX);
			assertEquals(nl+2, F_B_QX.NumLins());
		} catch (IOException e) {
			e.printStackTrace();
		}	
	}
	...
	public void testCmd_X() 
	{
		...
		tf = true;
        F_B_QX.setUpdateFlag(false);
        	
        try{
            RunC.Cmd_D(F_B_QX, 1);
			I_E_QX.Do_Update(F_B_QX);
			F_B_QX = new File_Buffer();
			I_E_QX = new Init_Exit(input_QX,F_B_QX);
			assertEquals("@Key", F_B_QX.GetLine(1));
			assertEquals(nl, F_B_QX.NumLins());
		} catch (IOException e){
			e.printStackTrace();
		}	
	}
	...
	@Test
	public void testCmd_H()
	{
		RunC.Cmd_H("S");
		
		assertEquals("S | Shows/Prints number of file lines starting at CLN (and DOES NOT change CLN!).\r\n"
				+ "", T_output.toString());
	}
	
	@Test
	public void testEasterEgg() throws Exception{
		ByteArrayInputStream in = new ByteArrayInputStream("dgu2021".getBytes());
		System.setIn(in);
